...
export async function readClaudeApiKey(environment: Record<string, string | undefined>): Promise<string | null> {
  ...
  try {
    ...
    if (typeof parsed === "object" && parsed !== null && "anthropicApiKey" in parsed && typeof parsed.anthropicApiKey === "string" && parsed.anthropicApiKey.trim()) return parsed.anthropicApiKey.trim();
  } catch {}
  return null;
}

export async function saveClaudeApiKey(apiKey: string): Promise<void> {
  if (!apiKey.trim()) throw new PluginBootstrapError("The Anthropic API key must be non-empty.");
  await mkdir(authDirectory(), { recursive: true, mode: 0o700 });
  await chmod(authDirectory(), 0o700);
  await writeFile(authPath(), `${JSON.stringify({ anthropicApiKey: apiKey.trim() })}\n`, { mode: 0o600 });
