...
export class CodexEngine implements ScanEngine {
  ...
  async checkAuth(env: Record<string, string | undefined>): Promise<EngineAuth> {
    const hasKey = ["OPENAI_API_KEY", "CODEX_API_KEY"].some((name) => Boolean(env[name]?.trim()));
    if (hasKey) return { method: "api_key", verified: false, engine: "codex" };
    return { method: "stored_credentials", verified: false, engine: "codex" };
  }

  async login(options: { apiKey?: string; env: Record<string, string | undefined>; signal?: AbortSignal }): Promise<{ success: boolean }> {
    if (options.apiKey === undefined) return { success: false };
    const result = await loginApiKey(this.codexCommand(), options.env, options.apiKey, options.signal);
